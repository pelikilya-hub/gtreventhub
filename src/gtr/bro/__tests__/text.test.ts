...
describe("база знаний в текстовом режиме", () => {
  it("вопросы «как/почему/что такое» уходят в базу знаний, а не в афишу", () => {
    for (const q of [
      "как забронировать стол",
      "почему депозит такой большой",
      "что такое GTR",
      "во сколько закрываются клубы",
      "можно ли платить картой",
      "расскажи про Патонг",
    ]) {
      const p = planOf(q);
      expect(p.kind, q).toBe("faq");
      if (p.kind === "faq") expect(p.question).toBe(q);
    }
  });

  it("афиша и база площадок вопрос не перехватывают", () => {
    expect(planOf("что сегодня в патонге").kind).toBe("search");
    expect(planOf("какие клубы в патонге").kind).toBe("venues");

...
export type Venue = {
  ...
  website: string;
  social: string;
  telegram?: string; // канал/бот площадки — пока редкость, но уже есть
  source: string;
  sourceType: string;
  confidence: string;
  status: string;
...
export type RoleId =
  ...
  | "visitor"
  | "venue";
/** Все роли для таблицы «Доступы и роли».
 *
 *  Список обязан совпадать с RoleId. Раньше в нём не было «Посетителя» и
 *  «Площадки», и таблица врала умолчанием: у площадки есть свой кабинет
 *  и реальные права — программа, заявки, паспорт, — но колонки для неё
 *  в таблице не существовало. Человек, открывший экран, чтобы понять,
 *  что может заведение, не находил ответа и делал вывод, что прав нет
 *  вовсе.
 *
 *  Третье поле — демо-площадка роли, четвёртое — инициалы значка. */
...
export type OrgRequest = {
  ...
  assigneeName?: string;
  ts: number;
  /** Доехала ли заявка до сервера. Ставится только на устройстве автора:
   *  «pending» — ещё везём, «failed» — сервер не принял и повторы не
   *  помогли. В KV этого поля нет: то, что лежит на сервере, доехало по
   *  определению. Нужно, чтобы человек видел разницу между «отправлено» и
   *  «показано у меня на экране». */
  sync?: "pending" | "failed";
};

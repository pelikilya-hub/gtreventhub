        ...
        if (up.message?.text) {
          ...
          if (cmd === "/start" || cmd === "/menu") {
            ...
            if (u) {
              ...
              await tgApi("sendMessage", {
                chat_id: chatId,
                parse_mode: "HTML",
                // Приглашение — первое, что человек видит от продукта.
                // Прежний текст давил конкурсом и датой призов и не
                // говорил, что делать дальше: у человека оставалось
                // ощущение рассылки. Теперь — что это, три шага и всё.
                text: [
                  "🪩 <b>GTR Event — ночной Пхукет в одном экране</b>",
                  "",
                  "Живая афиша острова: кто где играет сегодня и в выходные. 110 площадок, 312 артистов с их музыкой, бронь стола с предзаказом и маршрут вечера.",
                  "",
                  "<b>Как начать — три шага:</b>",
                  "",
                  "1. Нажми «Создать аккаунт» — имя, почта, пароль. Полминуты.",
                  "",
                  "2. Поставь на телефон как приложение:",
                  "    • iPhone: кнопка «Поделиться» → «На экран Домой»",
                  "    • Android: меню браузера → «Установить приложение»",
                  "",
                  "3. Вернись сюда и пришли свою почту одним сообщением — привяжу Telegram, и брони с ответами площадок будут приходить тебе в личку.",
                  "",
                  "Бесплатно. Дальше просто спрашивай — подскажу, куда пойти сегодня.",
                ].join("\n"),
                reply_markup: {
                  inline_keyboard: [
                    [{ text: "Создать аккаунт", url: `${APP_URL}/gtr/signup` }],
                    [{ text: "Что сегодня на острове", url: `${APP_URL}/gtr/tonight` }],
                  ],
                },
              });
            }
          ...
          if (!u) {
            ...
            await reply(
              chatId,
              [
                "Чтобы связать Telegram с аккаунтом — пришли сюда почту, на которую регистрировался. Одним сообщением, больше ничего не нужно.",
                "",
                "Аккаунта ещё нет? Создай по кнопке ниже и возвращайся.",
              ].join("\n"),

...
def describe(path: str):
    ...
    # Знак бывает двухцветным: у NORA рисунок оранжевый, а надпись под ним
    # чёрная, и на тёмном фоне пропадает ровно половина знака. Одной
    # средней яркостью это не поймать — считаем доли. Тонкая тёмная
    # обводка светлого знака даёт единицы процентов и в «смешанные» не
    # проходит; настоящая двухцветность — это четверть площади и больше.
    if plate is None:
        dark = sum(1 for v in lums if v < 55) / len(lums)
        light = sum(1 for v in lums if v > 170) / len(lums)
        if dark >= 0.25 and light >= 0.04:
            tone = "mixed"

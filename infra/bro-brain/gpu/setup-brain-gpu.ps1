...
# Контекст НА СЛОТ, а не всего: llama.cpp делит --ctx-size между слотами
# поровну. Системный промпт BRO вместе со схемами инструментов — около
# 10 800 токенов, и это нижняя граница, ниже которой мозг просто не видит
# своих правил. Прежняя раскладка давала 3072 на слот: промпт обрезался на
# каждом запросе, и модель отвечала наугад, а выглядело это как «глупая
# модель», а не как ошибка настройки.
$PROMPT_TOKENS = 10800
if ($vram -ge 16000) {
    $slots = 2; $perSlot = 16384
} elseif ($vram -ge 11000) {
    $slots = 1; $perSlot = 16384
} else {
    $slots = 1; $perSlot = 12288
    Write-Host "!! Видеопамяти немного — один слот: второй гость встанет в очередь."
    Write-Host "   Контекст урезать нельзя, иначе мозг не увидит собственных правил."
}
$ctx = $slots * $perSlot
if ($perSlot -lt $PROMPT_TOKENS + 1024) {
    Write-Host "!! Контекст на слот ($perSlot) меньше системного промпта — так нельзя." -ForegroundColor Red
}
Write-Host ">> Видеопамять: $vram МБ -> слотов $slots, контекст $perSlot на слот (всего $ctx)"
...
$TUNNEL = "gtr-brain"
# Сертификат Cloudflare кладём в рабочую папку, а не оставляем в профиле.
# Иначе постоянный адрес работает только у той учётной записи, которая
# однажды вошла в браузере: у второго пользователя и у SYSTEM (а под ним
# идёт автозапуск) профиль другой, сертификата нет, и туннель молча
# откатывается на временный адрес.
$cert = "$dir\cert.pem"
$userCert = "$env:USERPROFILE\.cloudflared\cert.pem"
if (-not (Test-Path $cert) -and (Test-Path $userCert)) {
    try { Copy-Item $userCert $cert -Force; Write-Host ">> Сертификат Cloudflare скопирован в $dir" } catch { }
}
...
# `cloudflared tunnel login` открывает браузер и ЖДЁТ БЕСКОНЕЧНО. В
# автоматическом скрипте это тупик: 27.08.2026 запуск на нём и завис, а
# снаружи это неотличимо от медленной загрузки модели. Поэтому вход
# делается только по явной просьбе, ключом -Login. Без него мозг просто
# поднимется на временном адресе — он рабочий, адрес доезжает сам.
if (-not (Test-Path $cert)) {
    if ($Login -and -not $Unattended) {
        Write-Host ""
        Write-Host "==============================================================="
        ...
        Write-Host " Скрипт ждёт, пока вы это сделаете."
        Write-Host "==============================================================="
        Write-Host ""
        Invoke-Cf tunnel login | Out-Null
        if (Test-Path $userCert) {
            try { Copy-Item $userCert $cert -Force } catch { }
        }
    } else {
        Write-Host ">> Сертификата Cloudflare нет — работаем на временном адресе."
        Write-Host "   Нужен постоянный? Запусти один раз с ключом -Login."
    }
}
...
if (Test-Path $cert) {
    ...
    Invoke-Cf --origincert $cert tunnel create $TUNNEL | Out-Null
    $label = if ($BrainHost) { $BrainHost } else { "brain" }
    $route = Invoke-Cf --origincert $cert tunnel route dns --overwrite-dns $TUNNEL $label
    ...
    $listed = (Invoke-Cf --origincert $cert tunnel list) -match [regex]::Escape($TUNNEL)
...
# Замер скорости сразу после старта.
#
# Сборка может подняться и работать на процессоре: слои не влезли в
# видеопамять, или бинарник не тот под эту карту. Отличить это по виду
# невозможно — сервер отвечает, /health зелёный, просто медленно. А
# «медленно» здесь означает, что воркер не дождётся ответа за свои 26
# секунд, и гость увидит отказ. Поэтому меряем и говорим прямо.
try {
    $probe = @{
        model = "qwen3-8b"
    ...
    $r = Invoke-RestMethod "http://127.0.0.1:8080/v1/chat/completions" -Method Post `
        -Headers @{ authorization = "Bearer $token" } -ContentType "application/json" `
        -Body $probe -TimeoutSec 180
    $tps = [math]::Round($r.timings.predicted_per_second, 1)
    Write-Host ">> Скорость генерации: $tps токенов/с"
    if ($tps -lt 20) {
        Write-Host ""
        Write-Host "!! Это скорость процессора, а не видеокарты." -ForegroundColor Yellow
        Write-Host "   Обычно означает, что слои не поместились в видеопамять и"
        Write-Host "   часть модели считается на CPU. Воркер ждёт ответа 26 секунд —"
        Write-Host "   на такой скорости он их не дождётся."
        Write-Host "   Пришли Claude эту строку и вывод окна llama-server."
        Write-Host ""
    }
} catch {
    Write-Host "!! Замер скорости не удался ($_) — не критично, идём дальше."
}
...
function Start-Tunnel {
    ...
    $a = if ($named) {
        @("--no-autoupdate", "--origincert", $cert, "tunnel", "run", "--url", "http://127.0.0.1:8080", $TUNNEL)
    } else {

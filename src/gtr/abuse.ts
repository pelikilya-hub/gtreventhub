...
/** Снять счётчик попыток. Нужен ровно в одном месте: когда админ выдал
 *  человеку новый пароль. К этому моменту тот уже несколько раз промахнулся
 *  старым и упёрся в защиту от перебора — без сброса новый пароль первые
 *  пять минут отвечал бы тем же «слишком много попыток», и выглядело бы
 *  это как «пароль опять не тот».
 *
 *  Окно фиксированное, поэтому чистим текущий слот и предыдущий: попытки
 *  человека почти наверняка легли в оба. */
export const clearLimit = async (
  ...
  if (!kv) return;
  const slot = Math.floor(Date.now() / (windowSec * 1000));
  await kv.delete(`rl:${bucket}:${who}:${slot}`);
  await kv.delete(`rl:${bucket}:${who}:${slot - 1}`);
};
...
export const LIMITS = {
  ...
  /** Код на телефон: по номеру. Живому человеку хватает двух-трёх
   *  попыток; выше — это либо перепутанный номер, либо чужой телефон,
   *  который заваливают сообщениями с нашего адреса. */
  otpSend: { hits: 3, windowSec: 900 } as Limit,
  /** Код на телефон: по адресу. Каждая отправка стоит денег, и счёт
   *  выставят нам — поэтому перебор номеров подряд ограничен отдельно. */
  otpSendIp: { hits: 10, windowSec: 3600 } as Limit,
  /** Ввод кода. Шесть цифр перебираются за миллион попыток; десяти в
   *  четверть часа для этого не хватит, а человеку хватит. */
  otpTry: { hits: 10, windowSec: 900 } as Limit,

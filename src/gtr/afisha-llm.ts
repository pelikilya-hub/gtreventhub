...
export function buildExtractPrompt(text: string, today: string): string {
  return [
    ...
    text,
    "=== КОНЕЦ ТЕКСТА ===",
    // Гасим режим рассуждений Qwen3. Без него модель уводит весь ответ в
    // reasoning_content, а content остаётся пустым: на длинной странице
    // размышление не заканчивается в пределах лимита токенов, и наружу
    // приходит пустота, неотличимая от «событий нет».
    "/no_think",
  ].join("\n");
}
